...
function application(robotatorIP) {
    ...
    var simpleIP = ROBOTATOR_IP.slice(0, -5); // On enlève le port puisque le flux vidéo est sur le port 1234
    ...
    function switchOff() {
        ...
        $.post(ROBOTATOR_IP + "/poweroff", { "info": "Power off" }, // juste pour info
        function (data) {
    ...
    $(".motor").on(BUTTON_UP, function () {
        ...
        if ($(this).hasClass("tourne")) {
            $.post(ROBOTATOR_IP + "/motor", { "direction": '7' } // 7 remet le servomotor à 0
            );
        }
        // Ne s'applique pas s'il s'agit du servomotor (on ne veut pas couper les autres moteurs juste parce que l'on a tourné...)
        else {
                $.post(ROBOTATOR_IP + "/motor", { "direction": 0 } // 0 stoppe les moteurs
                );
            }
    });
    ...
    $(".motor").on(BUTTON_DOWN, function () {
        ...
        $.post(ROBOTATOR_IP + "/motor", { "direction": $(this).attr("id") });
    });
    ...
    $("#config").on("click", function () {
        ...
        ROBOTATOR_IP + "/getNetworkMode", function (currentNetworkMode) {
    ...
    function changeNetworkMod(msg, data) {
        ...
        $.post(ROBOTATOR_IP + "/changeNetworkMode", { "networkMode": data.networkMode });
    ...
    $(".password").on("click", function () {
        ...
        $.post(ROBOTATOR_IP + "/password", { "newPassword": newPassword });
    });
}