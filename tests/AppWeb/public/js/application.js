...
function application(robotatorAddress) {
    ...
    var AUTONOMOUS_NETWORK_MODE = "autonomous";
    var LOCAL_NETWORK_MODE = "local";

    var ROBOTATOR_ADDRESS = robotatorAddress;

    var ROBOTATOR_IP = JSON.parse(localStorage.getItem("config")).ip;
    var VIDEO_STREAM_PORT = JSON.parse(localStorage.getItem("config")).videoStreamPort;

    // Permet d'écouter le bon évènement en fonction du client : browser ou app (app par défaut)
    var BUTTON_DOWN = "touchstart";
    var BUTTON_UP = "touchend";
    var BUTTON_OUT = "touchleave";
    ...
    var videoStream = 'http://' + ROBOTATOR_IP + ':' + VIDEO_STREAM_PORT + '/stream/video.mjpeg';
    ...
    var appLaunched = new Event("appLaunched");
    ...
    function switchOff() {
        ...
        $.post(ROBOTATOR_ADDRESS + "/poweroff", { "info": "Power off" }, // juste pour info
        function (data) {
    ...
    function switchButtonImage(image) {
        ...
        var buttonOn = !(image.getAttribute('src').indexOf('_on') === -1);
    ...
    $(".motor").on(BUTTON_UP, function () {
        ...
        if ($(this).hasClass("tourne")) {
            $.post(ROBOTATOR_ADDRESS + "/motor", { "direction": '7' } // 7 remet le servomotor à 0
            );
        }
        // Ne s'applique pas s'il s'agit du servomotor (on ne veut pas couper les autres moteurs juste parce que l'on a tourné...)
        else {
                $.post(ROBOTATOR_ADDRESS + "/motor", { "direction": 0 } // 0 stoppe les moteurs
                );
            }
    });
    ...
    $(".motor").on(BUTTON_DOWN, function () {
        ...
        $.post(ROBOTATOR_ADDRESS + "/motor", { "direction": $(this).attr("id") });
    });

    // Turbo
    $("#turbo").on("click", function () {

        var turboOn = switchButtonImage(this); // Change l'image du bouton et récupère son état (true/false)
        var turboState = turboOn ? TURBO_ON : TURBO_OFF; // Si le turbo est désormais allumé, l'état est à 255, sinon 180

        // Commande l'activation du turbo
        $.post(ROBOTATOR_ADDRESS + "/turbo", { "turboState": turboState });
    });
    ...
    $(".headlights").on("click", function () {
        ...
        var headlightsOn = switchButtonImage(this); // Change l'image du bouton et récupère son état allumé (true/false)
        var headlightsState = headlightsOn ? 1 : 0; // Si les phares sont désormais allumés, l'état est à 1, sinon 0

        // Commande l'activation des phares
        $.post(ROBOTATOR_ADDRESS + "/headlights", { "headlightsState": headlightsState });
    });
    ...
    $("#config").on("click", function () {
        ...
        ROBOTATOR_ADDRESS + "/getNetworkMode", function (currentNetworkMode) {
    ...
    function changeNetworkMod(msg, data) {
        ...
        $.post(ROBOTATOR_ADDRESS + "/changeNetworkMode", { "networkMode": data.networkMode });
    ...
    $(".password").on("click", function () {
        ...
        var newPassword = $(this).attr("data-password");

        // test de changement de mot de passe
        $.post(ROBOTATOR_ADDRESS + "/password", { "newPassword": newPassword });
    });
}